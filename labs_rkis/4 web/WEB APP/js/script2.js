...
//#endregion
    ...
    //#region Загрузка файлов
        ...
        _elem('.upload-files').addEventListener('click', function(){
            ...
            tdata.append('files', document.querySelector('input[name="files"]').files[0]);

            
            
            _post({url:`${HOST}/upload`, data: tdata}, function (responseText){
                fileTable = JSON.parse(responseText)
                for (r=0; r<fileTable.length; r++){
                    const element = fileTable[r]
                    ...
                    let name = document.createElement('td');
                    name.textContent = element.document.querySelector('input[name="files"]').value;
                    row.append(name)
